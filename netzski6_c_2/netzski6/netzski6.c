...
/* Main program */ int main(void)
{
	...
	integer f_inqu(inlist *), f_open(olist *), s_rsfe(cilist *), do_fio(integer *, char *, ftnlen), e_rsfe(void), f_clos(cllist *),
		s_rsle(cilist *), do_lio(integer *, integer *, char *, ftnlen),
		e_rsle(void), s_wsle(cilist *), e_wsle(void);
	/* Subroutine */ int s_paus(char *, ftnlen);
	integer s_wsfi(icilist *), e_wsfi(void), i_indx(char *, char *, ftnlen, ftnlen);
	double sqrt(doublereal), pow_dd(doublereal *, doublereal *);
	integer i_dnnt(doublereal *);
	double d_mod(doublereal *, doublereal *);
	integer s_wsfe(cilist *), e_wsfe(void);
	double cos(doublereal), tan(doublereal), atan(doublereal), log(doublereal), exp(doublereal), sin(doublereal);
	...
	extern /* Subroutine */ int decl_(doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *,
	...
	*/
	...
	for (nfil = 1; nfil <= i__1; ++nfil)
	{
		...
		{ // read the temmperatures from the netzskiy.tm3 file
			s_rsle(&io___49);
		...
		for (nyear = nstrtyr; nyear <= i__2; ++nyear)
		{
			...
			if (nointernet && !nofiles)
			{
				s_copy(place + (nsetflag << 3), fileo + 15, (ftnlen)8, (ftnlen)8);
				if (ndruk == 1 && nopendruk == 0)
				{
					/*             open temporary file */
					*(unsigned char *)filtmp = *(unsigned char *)drivlet;
					s_copy(filtmp + 1, ":/JK/", (ftnlen)5, (ftnlen)5);
					s_copy(filtmp + 6, place + (nsetflag << 3), (ftnlen)8, (ftnlen)8);
			...
			if (FindWinter)
			{
				...
				{
					if (MaxMinTemp < mint[iTemp])
						MaxMinTemp = mint[iTemp];
				}
		...
		L40:
			...
			if (nazn == 1)
			{
				...
				real elevazi = 0;
				i__3 = do_lio(&c__4, &c__1, (char *)&elevazi, (ftnlen)sizeof(real));
				if (i__3 != 0)
				{
					goto L50;
				}
			...
			{
				i__3 = do_lio(&c__4, &c__1, (char *)&elev[(nazn << 2) - 4], (ftnlen)sizeof(real));
				if (i__3 != 0)
				{
					goto L50;
				}
			}
			i__3 = do_lio(&c__4, &c__1, (char *)&elev[(nazn << 2) - 3], (ftnlen)sizeof(real));
			if (i__3 != 0)
			{
				goto L50;
			}
			i__3 = do_lio(&c__5, &c__1, (char *)&e2, (ftnlen)sizeof(doublereal));
			if (i__3 != 0)
			{
				goto L50;
			}
			i__3 = do_lio(&c__5, &c__1, (char *)&e3, (ftnlen)sizeof(doublereal));
			if (i__3 != 0)
			{
				goto L50;
			}
			i__3 = do_lio(&c__4, &c__1, (char *)&elev[(nazn << 2) - 2], (ftnlen)sizeof(real));
			if (i__3 != 0)
			{
				goto L50;
			}
			i__3 = do_lio(&c__4, &c__1, (char *)&elev[(nazn << 2) - 1], (ftnlen)sizeof(real));
			if (i__3 != 0)
			{
				goto L50;
			}
		...
		L50:
				...
				*/
				...
				if (flagdynum) // use the actual ground temperatures and pressures instead
				{
					if (stream = fopen(fndynum, "r"))
					{
						...
						fclose(stream);
						if (tk != -9999)
							goto L500; // skip temperature definition via WorldClim temperatures
					}
				}
			...
			L500:
					...
					if (d_mod(&dy, &c_b177) == 0.)
					{
						...
						*(unsigned char *)filstat = *(unsigned char *)drivlet;
						...
						if (nstat <= 9)
						{
							...
							e_wsfi();
							*(unsigned char *)&filstat[13] = *(unsigned char *)ch1;
						}
				...
				ref = exp(vdwref[0] + vdwref[1] * lnhgt + vdwref[2] * lnhgt * lnhgt + vdwref[3] * lnhgt * lnhgt * lnhgt + vdwref[4] * lnhgt * lnhgt * lnhgt * lnhgt + vdwref[5] * lnhgt * lnhgt * lnhgt * lnhgt * lnhgt);
				eps = exp(vdweps[0] + vdweps[1] * lnhgt + vdweps[2] * lnhgt * lnhgt + vdweps[3] * lnhgt * lnhgt * lnhgt + vdweps[4] * lnhgt * lnhgt * lnhgt * lnhgt);
			...
			L700:
				...
				if (nsetflag == 1)
				{
					...
					if (adhocset)
					{
						...
						{
							if (lt >= 0. && nweather == 0 && (dy <= (doublereal)ns3 || dy >= (doublereal)ns4))
							{
					...
					if (astro)
					{
						...
						if (!nofiles && nointernet)
						{
							...
							{
								s_wsle(&io___183);
								do_lio(&c__9, &c__1, "sunset for year ", (ftnlen)16);
								do_lio(&c__5, &c__1, (char *)&yr, (ftnlen)sizeof(doublereal));
							...
							do_fio(&c__1, (char *)&azi1, (ftnlen)sizeof(doublereal));
							////////////////081021/////add viewangle/////////////
							viewangle = 0.0;
							do_fio(&c__1, (char *)&viewangle, (ftnlen)sizeof(doublereal));
							//////////////////////////////////////////////
							do_fio(&c__1, (char *)&dobs, (ftnlen)sizeof(doublereal));
							e_wsfe();
						}
						goto L920;
					}
				...
				else if (nsetflag == 0)
				{
					...
					if (astro)
					{
						...
						if (!nofiles && nointernet)
						{
							...
							{
								s_wsle(&io___193);
								do_lio(&c__9, &c__1, "sunrise for year ", (ftnlen)17);
								do_lio(&c__5, &c__1, (char *)&yr, (ftnlen)sizeof(doublereal));
								do_lio(&c__9, &c__1, "place: ", (ftnlen)7);
								do_lio(&c__9, &c__1, fileo, (ftnlen)27);
								e_wsle();
			...
			L692:
				...
				i__3 = nlpend;
				i__4 = ndirec;
				for (nloops = nloop; i__4 < 0 ? nloops >= i__3 : nloops <= i__3; nloops += i__4)
				{
					++nn;
					...
					z__ = sin(lr) * sin(d__) + cos(lr) * cos(d__) * cos(hran / ch); // zenith angle
					z__ = acos(z__);
					...
					if (nsetflag == 1)
					{
						...
						if (!nweatherflag)
						{
							...
							vbwexp = 1.68271708343173 - 1.04055307822257E-04 * hgt - 4.65108719859762E-03 * altmin + 1.47797248203399E-05 * altmin * altmin - 2.37213585737878E-08 * altmin * altmin * altmin + 1.48109107804038E-11 * altmin * altmin * altmin * altmin;
							vbwexp += ErrorFudge;
					...
					L695:
						...
						elevintx = elev[(ne + 1 << 2) - 4] - elev[(ne << 2) -
																  4];
						elevint = elevinty / elevintx * (azi1 - elev[(ne << 2) - 4]) + elev[(ne << 2) - 3];
						...
						else
						{
							...
							hgtdifx = elev[(ne + 1 << 2) - 1] - elev[(ne << 2) -
																	 1];
							hgtdif = hgtdifx / elevintx * (azi1 - elev[(ne << 2) - 4]) + elev[(ne << 2) - 1];
							hgtdif -= hgt;
							// convert to kms
							hgtdif *= .001f;
							/* 			   now calculate interpolated distance to obstruction along earth */
							distx = elev[(ne + 1 << 2) - 2] - elev[(ne << 2) - 2];
							dist = distx / elevintx * (azi1 - elev[(ne << 2) - 4]) + elev[(ne << 2) - 2];
							/* 			   now calculate pathlength of curved ray using Lehn's parabolic approximation */
							/* Computing 2nd power */
							d__2 = dist;
							/* Computing 2nd power */
						...
						if ((elevint >= al1 || nloops <= 0) && nabove == 1)
						{
							...
							if (nointernet && !nofiles)
							{
								...
								t3sub = (d__2 = (stepsec * nloops - nacurr) /
												3600.,
										 abs(d__2)); // difference in time using ast sunset without adhoc fix as reference
								...
								{
									s_wsle(&io___219);
									do_lio(&c__5, &c__1, (char *)&dy, (ftnlen)sizeof(doublereal));
									do_lio(&c__5, &c__1, (char *)&t3sub, (ftnlen)sizeof(doublereal));
									e_wsle();
								}
								t1500_(&t3sub, &negch);
								s_copy(ts2c, "         ", (ftnlen)9, (ftnlen)9);
								s_copy(ts2c + 1, t3s_1.t3subc, (ftnlen)8, (ftnlen)8);
								if (nloops > 0)
								{
									*(unsigned char *)ts2c = '-';
								}
							}
							t3sub = t3 + ndirec * (stepsec * nloops + nacurr) / 3600.; // this is visible sunset with adhoc fix
							if (nloops < 0)
							{
								/*                     visible sunset can't be later then astronomical sunset! */
								t3sub = t3 + nacurr / 3600;
							...
							/*                 <<<<<<<<<distance to obstruction>>>>>> */
							dobsy = elev[(ne + 1 << 2) - 2] - elev[(ne << 2) - 2];
							elevintx = elev[(ne + 1 << 2) - 4] - elev[(ne << 2) - 4];
							dobs = dobsy / elevintx * (azi1 - elev[(ne << 2) - 4]) + elev[(ne << 2) - 2];
					...
					L760:
						...
						if (elevint <= alt1)
						{
							...
							{
								s_wsle(&io___226);
								do_lio(&c__9, &c__1, "sunrise for year: ", (ftnlen)18);
								do_lio(&c__3, &c__1, (char *)&nyr, (ftnlen)sizeof(integer));
			...
			L920:
				if (nfil == 1)
				{
					...
					*/
					...
					tsscs[nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7] =
						t3sub;
					s_copy(coments + (nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7) * 12, comentss, (ftnlen)12, (ftnlen)12);
				...
				else
				{
					...
					{
						/*              sunrise */
						if (t3sub < tsscs[nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7])
						{
							...
							*/
							...
							tsscs[nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7] = t3sub;
							s_copy(coments + (nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7) * 12, comentss, (ftnlen)12, (ftnlen)12);
							azils[nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7] = azi1;
							dobss[nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7] = dobs;
							viewang[nsetflag + 1 + (nyrstp + (ndystp << 1) << 1) - 7] = viewangle;
						}
					}
					else if (nsetflag == 1)
					{
...
/* Subroutine */ int t1750_(doublereal *dy, integer *nyl, integer *nyr)
{
	...
	else if (ndy > 31 && ((!nleap && ndy <= 59) || (nleap && ndy <= 60)))
	{
		s_copy(t3s_1.caldayc, "Feb-", (ftnlen)4, (ftnlen)4);
		s_wsfi(nleap ? &io___277 : &io___276);
		i__1 = ndy - 31;
...
/* Subroutine */ int refvdw_(doublereal *hgt, doublereal *x, doublereal *fref, const short *nweatherflag, const double *vdwsf)
{
	...
	*/
	...
	ca[0] = coef[0] + coef[1] * *hgt + coef[2] * *hgt * *hgt + coef[3] * *hgt * *hgt * *hgt + coef[4] * *hgt * *hgt * *hgt * *hgt;
	ca[1] = coef[5] + coef[6] * *hgt + coef[7] * *hgt * *hgt + coef[8] * *hgt * *hgt * *hgt + coef[9] * *hgt * *hgt * *hgt * *hgt;
	ca[2] = coef[10] + coef[11] * *hgt + coef[12] * *hgt * *hgt + coef[13] * *hgt * *hgt * *hgt + coef[14] * *hgt * *hgt * *hgt * *hgt;
	ca[3] = coef[15] + coef[16] * *hgt + coef[17] * *hgt * *hgt + coef[18] * *hgt * *hgt * *hgt + coef[19] * *hgt * *hgt * *hgt * *hgt;
	ca[4] = coef[20] + coef[21] * *hgt + coef[22] * *hgt * *hgt + coef[23] * *hgt * *hgt * *hgt + coef[24] * *hgt * *hgt * *hgt * *hgt;
	ca[5] = coef[25] + coef[26] * *hgt + coef[27] * *hgt * *hgt + coef[28] * *hgt * *hgt * *hgt + coef[29] * *hgt * *hgt * *hgt * *hgt;
	ca[6] = coef[30] + coef[31] * *hgt + coef[32] * *hgt * *hgt + coef[33] * *hgt * *hgt * *hgt + coef[34] * *hgt * *hgt * *hgt * *hgt;
	ca[7] = coef[35] + coef[36] * *hgt + coef[37] * *hgt * *hgt + coef[38] * *hgt * *hgt * *hgt + coef[39] * *hgt * *hgt * *hgt * *hgt;
	ca[8] = coef[40] + coef[41] * *hgt + coef[42] * *hgt * *hgt + coef[43] * *hgt * *hgt * *hgt + coef[44] * *hgt * *hgt * *hgt * *hgt;
	ca[9] = coef[45] + coef[46] * *hgt + coef[47] * *hgt * *hgt + coef[48] * *hgt * *hgt * *hgt + coef[49] * *hgt * *hgt * *hgt * *hgt;
	ca[10] = coef[50] + coef[51] * *hgt + coef[52] * *hgt * *hgt + coef[53] * *hgt * *hgt * *hgt + coef[54] * *hgt * *hgt * *hgt * *hgt;
	*fref = ca[0] + ca[1] * *x + ca[2] * *x * *x + ca[3] * *x * *x * *x + ca[4] * *x * *x * *x * *x + ca[5] * *x * *x * *x * *x * *x + ca[6] * *x * *x * *x * *x * *x * *x + ca[7] * *x * *x * *x * *x * *x * *x * *x + ca[8] * *x * *x * *x * *x * *x * *x * *x * *x + ca[9] * *x * *x * *x * *x * *x * *x * *x * *x * *x + ca[10] * *x * *x * *x * *x * *x * *x * *x * *x * *x * *x;
	return 0;
} /* refvdw_ */
...
//////////////////////////////////////////////////////////////////
{
	...
	while (token != NULL)
	{
		...
		if (token == NULL)
			return -1; // can't find token in the string
		numtoken++;
